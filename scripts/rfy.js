...
function goToSlickSlidePage(pageNum, animate=true) {
  console.log('goToslickSlidePage:', pageNum, "animate:", animate);
  curSlick = pageNum;
  ...
  if (ulSlickDots8.is(":visible")) {
    ...
    if (event.pageX < curLiLeft) {
      if (curSlick !== 1) {
        ...
        // console.log('rfyWrapper onclick go prev');
        goToSlickSlidePage(curSlick - 1, false);
      }
    } else if (event.pageX > curLiRight) {
      if (curSlick !== slickLength) {
        // console.log('rfyWrapper onclick go next');
        goToSlickSlidePage(curSlick + 1, false);
      }
    }
  }
